...
public class task_1BiFile {
    ...
    public static void main(String[] args) {
        ...
        try {
            add("key1", "значение 1"); // Добавления условного ключа и, что будет храниться под этим ключом.
            add("key2", "значение 2"); // Добавления условного ключа и, что будет храниться под этим ключом.
            add("key4", "значение 3"); // Добавления условного ключа и, что будет храниться под этим ключом.
            saveFile(file); // Схранение файла.
        } catch (Exception e) {
            e.printStackTrace(); // Выдаст ошибку если мы не смогли выполнить предыдущий блок.
        }
        //read file
        
        try {
            loadFile(file);
            String key1 = "key1"; // По ключам пытаемся найти какие то значения.
            String key2 = "key2"; // По ключам пытаемся найти какие то значения.
            String key3 = "key3"; // По ключам пытаемся найти какие то значения. Но для понимания
                                  // ищем ключ "key3", а в предыдущем блоке записывали ключ "key4"
                                  // т.е. функция loadFile(file) вернёт сообщение "не найдено".
            System.out.printf("%s: %s\n", key1, find(key1)); // Получим на выводе значения дыннх ключей.
            System.out.printf("%s: %s\n", key2, find(key2)); // Получим на выводе значения дыннх ключей.
            System.out.printf("%s: %s\n", key3, find(key3)); // Получим на выводе сообщение "не найдено".
        } catch (IOException e) {
            e.printStackTrace(); // Выдаст ошибку если мы не смогли выполнить предыдущий блок.
        }
    }
